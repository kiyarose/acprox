...
function loadcustomapp() {
  ...
  if (name.length > 15)
    return alert("App name is too long (max 30 characters)");
      ...
      customapps.push(
        JSON.parse(
          `{ "title": "${name} (Custom app)", "url": "${url}", "id": "${
            data[0]
          }", "image": "${icon}", "description": "${description}" }`,
        ),
      );
...
function loadcustomgame() {
  ...
  if (!name || !url) return alert("All required fields must be filled in");
  if (name.length > 15)
    return alert("Game name is too long (max 30 characters)");
  ...
  fetch("https://www.uuidtools.com/api/generate/v4")
    ...
    .then((data) => {
      ...
      customgames.push(
        JSON.parse(
          `{ "title": "${name} (Custom game)", "url": "${url}", "id": "${
            data[0]
          }", "image": "${icon}", "description": "${description}" }`,
        ),
      );
